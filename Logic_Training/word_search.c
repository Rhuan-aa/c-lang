...
bool backtrack(char** board, int boardSize, int* boardColSize, int row, int col, char* suffix) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (backtrack(board, boardSize, boardColSize, row + row_offset, col + col_offset, suffix + 1))  return true;
    }
...
// Aprendizado interessante "char[] + int" aumenta a casa inicial que será lida de inicio com base no 'int'